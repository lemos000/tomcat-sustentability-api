...
public class ProjetosSustentaveisDAO {
    ...
    public boolean inserir(ProjetoSustentavel projeto) throws SQLException {
        String sql = "INSERT INTO tb_PROJETOS_SUSTENTAVEIS (descricao, custo, status, id_tipo_fonte, id_regiao) VALUES (?, ?, ?, ?, ?)";
    ...
    public String atualizar(ProjetoSustentavel projeto) throws SQLException {
        String sql = "UPDATE tb_PROJETOS_SUSTENTAVEIS SET descricao = ?, custo = ?, status = ?, id_tipo_fonte = ?, id_regiao = ? WHERE id_projeto = ?";
    ...
    public ProjetoSustentavel selecionar(int id) throws SQLException {
        PreparedStatement stmt = minhaConexao.prepareStatement(
                "SELECT * FROM tb_PROJETOS_SUSTENTAVEIS WHERE id_projeto = ?");
        stmt.setInt(1, id);
    ...
    public List<ProjetoSustentavel> selecionarTodos() throws SQLException {
        ...
        PreparedStatement stmt = minhaConexao.prepareStatement(
                "SELECT * FROM tb_PROJETOS_SUSTENTAVEIS ORDER BY id_projeto");
        ResultSet rs = stmt.executeQuery();
